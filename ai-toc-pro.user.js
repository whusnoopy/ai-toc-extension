...
/* 折叠后的状态：高度缩减到 Header 的高度 */
#ai-toc-root.collapsed {
  max-height: 40px !important;
}

.ai-toc-header {
  height: 40px;
  min-height: 40px; /* 确保折叠后高度稳定 */
  padding: 0 10px;
  display: flex;
  align-items: center;
...
#ai-toc-toggle-btn {
  ...
  padding: 4px; border-radius: 4px; display: flex; transition: transform 0.3s; opacity: 0.5;
}
#ai-toc-toggle-btn:hover { background: var(--toc-hover); opacity: 1; }

/* 折叠时旋转图标 */
.collapsed #ai-toc-toggle-btn {
  transform: rotate(-90deg);
}

.ai-toc-title {
  font-size: 11px; font-weight: 700; text-transform: uppercase;
  opacity: 0.6; letter-spacing: 0.8px; flex: 1; margin-left: 6px;
}

.ai-toc-actions { display: flex; align-items: center; gap: 2px; }

#ai-toc-refresh-btn {
  background: none; border: none; cursor: pointer; color: var(--toc-text);
  padding: 4px; border-radius: 4px; display: flex; transition: all 0.2s; opacity: 0.5;
}
#ai-toc-refresh-btn:hover { background: var(--toc-hover); opacity: 1; }

.ai-toc-content { overflow-y: auto; padding: 4px 0; flex: 1; }

/* 内容折叠时隐藏 */
.collapsed .ai-toc-content {
  display: none;
}
...
.ai-toc-content::-webkit-scrollbar-thumb { background: rgba(0, 0, 0, 0.1); border-radius: 10px; }
  ...
  function renderTOC() {
    ...
    if (!container) return;
    if (tocData.length === 0) {
      container.innerHTML = safeHTML('<div style="padding:20px;text-align:center;font-size:11px;opacity:0.4;">等待内容生成...</div>');
      return;
    }
